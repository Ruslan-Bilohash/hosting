...
function parsePhpLang(filePath) {
  const data = {};
  for (const line of fs.readFileSync(filePath, 'utf8').split(/\r?\n/)) {
    const m = line.match(LINE_RE);
    if (m) data[m[1]] = m[2].trim().replace(/,$/, '');
  }
  return data;
}
...
function writePhpLang(filePath, data, order) {
  const lines = ['<?php', 'return ['];
  const seen = new Set();
  for (const key of order) {
    if (data[key] !== undefined && !seen.has(key)) {
      lines.push(`    '${key}' => ${data[key]},`);
      seen.add(key);
    }
  }
  for (const key of Object.keys(data).sort()) {
    if (!seen.has(key)) lines.push(`    '${key}' => ${data[key]},`);
  }
  lines.push('];');
...
function mergePanel(target, ref, translations, label) {
  const added = [];
  const failed = [];
  ...
  if (failed.length) console.error(`FAILED ${label}:`, failed.join(', '));
  return { added, failed };
}

const panelUk = parsePhpLang(path.join(LANG, 'panel-uk.php'));
const panelEn = parsePhpLang(path.join(LANG, 'panel-en.php'));
const panelNo = parsePhpLang(path.join(LANG, 'panel-no.php'));

const before = {
  'panel-uk': Object.keys(panelUk).length,
  'panel-en': Object.keys(panelEn).length,
  'panel-no': Object.keys(panelNo).length,
  'public-uk': Object.keys(parsePhpLang(path.join(LANG, 'uk.php'))).length,
  'public-en': Object.keys(parsePhpLang(path.join(LANG, 'en.php'))).length,
  'public-no': Object.keys(parsePhpLang(path.join(LANG, 'no.php'))).length,
};
console.log('BEFORE:', before);

const rEn = mergePanel(panelEn, panelUk, PANEL_EN, 'panel-en');
const rNo = mergePanel(panelNo, panelUk, PANEL_NO, 'panel-no');

const ukOrder = Object.keys(panelUk);
writePhpLang(path.join(LANG, 'panel-en.php'), panelEn, ukOrder);
writePhpLang(path.join(LANG, 'panel-no.php'), panelNo, ukOrder);

const after = {
  'panel-uk': Object.keys(panelUk).length,
  'panel-en': Object.keys(parsePhpLang(path.join(LANG, 'panel-en.php'))).length,
  'panel-no': Object.keys(parsePhpLang(path.join(LANG, 'panel-no.php'))).length,
  'public-uk': Object.keys(parsePhpLang(path.join(LANG, 'uk.php'))).length,
  'public-en': Object.keys(parsePhpLang(path.join(LANG, 'en.php'))).length,
  'public-no': Object.keys(parsePhpLang(path.join(LANG, 'no.php'))).length,
};
console.log('AFTER:', after);
console.log('panel-en added:', rEn.added.length);
console.log('panel-no added:', rNo.added.length);

const failed = [...rEn.failed, ...rNo.failed];
if (failed.length) {
  console.error('UNTRANSLATED:', failed);
  process.exit(1);
}
