...
$admin_nav_active = $tab === 'services' ? 'services' : 'plans';
ob_start();
?>
        ...
        <div class="hp-card-body">
          <div class="hs-table-wrap">
            <table class="hs-table">
              <thead><tr>
                <th><?= hs_h($t['admin_col_id'] ?? 'ID') ?></th><th><?= hs_h($t['plan_price'] ?? 'Price') ?></th><th><?= hs_h($t['plan_websites_limit'] ?? 'Sites') ?></th><th><?= hs_h($t['plan_storage_limit'] ?? 'Storage') ?></th><th></th>
              </tr></thead>
              <tbody>
              <?php foreach ($catalog['plans'] as $pid => $plan): ?>
                <tr>
                  <td><code><?= hs_h($pid) ?></code> <?= empty($plan['active']) ? '<span class="hs-badge-muted">' . hs_h($t['admin_badge_off'] ?? 'off') . '</span>' : '' ?></td>
      ...
      <section class="hp-card hs-admin-form-card">
        ...
        <div class="hp-card-body">
          <form method="post" class="hs-admin-form">
            ...
            <div class="hp-grid-2">
              <div class="hs-field"><label><?= hs_h($t['admin_col_id'] ?? 'ID') ?></label><input type="text" name="id" value="<?= hs_h((string) ($p['id'] ?? $editPlan)) ?>" <?= $newPlan ? '' : 'readonly' ?> pattern="[a-z0-9_-]+" required></div>
              ...
              <div class="hs-field"><label><?= hs_h($t['admin_field_badge'] ?? 'Badge') ?></label><input type="text" name="badge" value="<?= hs_h((string) ($p['badge'] ?? '')) ?>" placeholder="popular / vps"></div>
              <div class="hs-field"><label><?= hs_h($t['admin_field_sort'] ?? 'Sort') ?></label><input type="number" name="sort" value="<?= hs_h((string) ($p['sort'] ?? '0')) ?>"></div>
    ...
    <div class="hs-admin-split">
      <section class="hp-card">
        ...
        <div class="hp-card-body">
          <div class="hs-table-wrap">
            <table class="hs-table">
              <thead><tr><th><?= hs_h($t['admin_col_id'] ?? 'ID') ?></th><th><?= hs_h($t['admin_services_name'] ?? 'Name') ?></th><th><?= hs_h($t['plan_price'] ?? 'Price') ?></th><th></th></tr></thead>
              <tbody>
              <?php foreach ($catalog['services'] as $svc): ?>
                <tr>
                  ...
                  <td>
                    ...
                    <form method="post" style="display:inline" onsubmit="return confirm(<?= json_encode($t['admin_confirm_delete'] ?? 'Delete?', JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE) ?>)"><?= hs_csrf_field() ?><input type="hidden" name="delete_service" value="1"><input type="hidden" name="service_id" value="<?= hs_h((string) ($svc['id'] ?? '')) ?>"><button type="submit" class="hs-btn hs-btn-ghost hp-dash-btn-sm"><i class="fa-solid fa-trash"></i></button></form>
                  </td>
                </tr>
              <?php endforeach; ?>
              </tbody>
      ...
      <section class="hp-card hs-admin-form-card">
        ...
        <div class="hp-card-body">
          <form method="post" class="hs-admin-form">
            ...
            <div class="hs-field"><label><?= hs_h($t['admin_col_id'] ?? 'ID') ?></label><input type="text" name="id" value="<?= hs_h((string) ($s['id'] ?? $editService)) ?>" <?= $newService ? '' : 'readonly' ?> pattern="[a-z0-9_-]+" required></div>
            <div class="hs-field"><label><?= hs_h($t['admin_field_icon'] ?? 'Icon (FA)') ?></label><input type="text" name="icon" value="<?= hs_h((string) ($s['icon'] ?? 'fa-puzzle-piece')) ?>"></div>
            ...
            <div class="hs-field"><label><?= hs_h($t['admin_plans_active'] ?? 'Active') ?></label><label class="hs-check"><input type="checkbox" name="active" value="1" <?= empty($s) || !empty($s['active']) ? 'checked' : '' ?>> <?= hs_h($t['admin_badge_on'] ?? 'on') ?></label></div>
            <?php foreach (['uk', 'en', 'no'] as $lng): ?>
              <div class="hs-field"><label><?= hs_h($t['admin_services_name'] ?? 'Name') ?> (<?= hs_h(strtoupper($lng)) ?>)</label><input type="text" name="label_<?= hs_h($lng) ?>" value="<?= hs_h((string) (($s['labels'][$lng] ?? ''))) ?>"></div>
              <div class="hs-field"><label><?= hs_h($t['admin_field_desc'] ?? 'Description') ?> (<?= hs_h(strtoupper($lng)) ?>)</label><input type="text" name="desc_<?= hs_h($lng) ?>" value="<?= hs_h((string) (($s['desc'][$lng] ?? ''))) ?>"></div>
            <?php endforeach; ?>
            <button type="submit" class="hs-btn hs-btn-primary"><?= hs_h($t['admin_save'] ?? 'Save') ?></button>
          </form>
        </div>
